import { formatCurrency } from "@/lib/formatCurrency";import { imageUrl } from "@/lib/imageUrl";
;
                                    ...
                                    <div className="p-4 sm:p-6 border-b border-gray-200">
                                        ...
                                        <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
                                            <div className="flex items-center">
                                                ...
                                                {order.status ? order.status.charAt(0).toUpperCase() + order.status.slice(1) : order.status}
                                                </ span>
                                            </div>
                                            <div className="sm: text-right">
                                            ...
                                            <div className="mt-4 p-3 sm:p-4 bg-red-50 rounded-lg">
                                                ...
                                                <p className="text-sm text-gray-600">
                                                    Original Subtotal: {" "} {formatCurrency(
                                                        (order.totalAmount ?? 0) + order.amountDiscount, order.currency)}</p>
                                            </div>
                                        ) : null}
                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-3 sm:gap-4">
                                                        {product.product?.images && (
                                                            <div className="relative h-14 w-14 sm:h-16 sm:w-16 flex-shrink-0 rounded-md overflow-hidden">
                                                                <Image src={imageUrl(product.product.images[0]).url()}
                                                                    alt={product.product?.name ?? ''} className="object-cover" fill
                                                                />
                                                            </div>
                                                        )}
                                                        <div>
